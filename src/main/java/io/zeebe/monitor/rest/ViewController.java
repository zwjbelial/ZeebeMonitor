...
public class ViewController {
     ...
     * @return
     */
    public WorkflowInstanceListDto toDto(WorkflowInstanceEntity instance) {
        ...
        dto.setStartTime(Instant.ofEpochMilli(instance.getStart()).atOffset(ZoneOffset.ofHours(8)).toString());
        //增加开始时间存储
        dto.setStartDate(new Date(instance.getStart()));

        if (isEnded) {
            dto.setEndTime(Instant.ofEpochMilli(instance.getEnd()).atOffset(ZoneOffset.ofHours(8)).toString());
            dto.setEndDate(new Date(instance.getEnd()));
        }
    ...
    //TODO 获取instaces中所有tab的属性,其中jobDtos为每个job的状态
    ...
    public WorkflowInstanceDto toInstanceDto(WorkflowInstanceEntity instance) {
                ...
                events.stream()
                        .map(
                                e -> {
                                    ...
                                    entry.setTimestamp(Instant.ofEpochMilli(e.getTimestamp()).atOffset(ZoneOffset.ofHours(8)).toString());
                ...
                incidents.stream()
                        .map(
                                i -> {
                                    ...
                                    incidentDto.setCreatedTime(Instant.ofEpochMilli(i.getCreated()).atOffset(ZoneOffset.ofHours(8)).toString());

                                    if (isResolved) {
                                        incidentDto.setResolvedTime(Instant.ofEpochMilli(i.getResolved()).atOffset(ZoneOffset.ofHours(8)).toString());
        ...
        variablesByScopeAndName.forEach(
                (scopeKeyName, variables) -> {
                    ...
                    variableDto.setTimestamp(Instant.ofEpochMilli(lastUpdate.getTimestamp()).atOffset(ZoneOffset.ofHours(8)).toString());
                            ...
                            variables.stream()
                                    .map(
                                            v -> {
                                                ...
                                                varUpdate.setTimestamp(Instant.ofEpochMilli(v.getTimestamp()).atOffset(ZoneOffset.ofHours(8)).toString());
                                                return varUpdate;
                                            })
                                    .collect(Collectors.toList());
    ...
    private IncidentListDto toDto(IncidentEntity incident) {
        ...
        dto.setCreatedTime(Instant.ofEpochMilli(incident.getCreated()).atOffset(ZoneOffset.ofHours(8)).toString());

        if (isResolved) {
            dto.setResolvedTime(Instant.ofEpochMilli(incident.getResolved()).atOffset(ZoneOffset.ofHours(8)).toString());
    ...
    private JobDto toDto(JobEntity job) {
        ...
        dto.setTimestamp(Instant.ofEpochMilli(job.getTimestamp()).atOffset(ZoneOffset.ofHours(8)).toString());

        dto.setStartDate(new Date(job.getTimestamp()));
        return dto;
    }
    ...
    private MessageDto toDto(MessageEntity message) {
        ...
        dto.setTimestamp(Instant.ofEpochMilli(message.getTimestamp()).atOffset(ZoneOffset.ofHours(8)).toString());
    ...
    private MessageSubscriptionDto toDto(MessageSubscriptionEntity subscription) {
        ...
        dto.setTimestamp(Instant.ofEpochMilli(subscription.getTimestamp()).atOffset(ZoneOffset.ofHours(8)).toString());
    ...
    private TimerDto toDto(TimerEntity timer) {
        ...
        dto.setDueDate(Instant.ofEpochMilli(timer.getDueDate()).atOffset(ZoneOffset.ofHours(8)).toString());
        dto.setTimestamp(Instant.ofEpochMilli(timer.getTimestamp()).atOffset(ZoneOffset.ofHours(8)).toString());
